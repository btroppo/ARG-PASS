...
# Commands are intended to be executed step-by-step manually. Potential qARP APH's with best 0-25% seqID to known ARGs are used as an example throughout
...
cd $REPO_ROOT/example/APH/cluster_20/AF-Q08JA6-F1-model_v6
...
cd $REPO_ROOT/example/APH/cluster_20/AF-Q08JA6-F1-model_v6/high_lddt_subjects
mkdir -p database
...
# Step 6: Get highest seqID of query. pdb files to all ARP structures of ARG class, for cluster level assignment (see Methods of paper). In this example the queries are assigned to the 20% seqID clusters of APH's
cd $REPO_ROOT/example/APH
mkdir -p database
foldseek createdb *.pdb database/ARP_subjects.pdb
foldseek easy-search *.pdb database/ARP_subjects.pdb queries/queries_subjects.out tmp --exhaustive-search --format-output "query,target,alntmscore,qtmscore,ttmscore,pident,qlen,alnlen,tlen"
...
# Results (functional predictions.csv and visualisation figures) for the cluster are saved to $REPO_ROOT/example/APH/cluster_20/AF-Q08JA6-F1-model_v6/high_lddt_subjects/output 
